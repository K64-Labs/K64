...
MONITOR = "/tmp/k64-sl-monitor.sock"
SERIAL_PORT = 46064


def kill_old_qemu():
    proc = subprocess.run(["pgrep", "-x", "qemu-system-x86"], text=True, capture_output=True)
...
def main():
    ...
    cmd = [
        ...
        "-cdrom", "k64.iso",
        "-boot", "d",
        "-netdev", "user,id=k64net",
        "-device", "rtl8139,netdev=k64net",
        "-display", "gtk",
    ...
    read_until(serial, ">>>", 5)
    send_slow(serial, "klcs run sl\n")
    read_until(serial, "dynamic x86_64 ELF accepted", 10)
    time.sleep(1.1)

    shot = ROOT / "build" / "sl-running.ppm"
    monitor_command(f"screendump {shot}")
    print(f"SCREENSHOT={shot}")

    try:
        read_until(serial, "ELF: exit code", 20)
        print("SL_EXIT=ok")
    except Exception as exc:
        print(f"SL_EXIT_WAIT={exc}")
